...
void main() {
  testWidgets('Selected date renders as "Today" when it is DateTime.now()', (
    tester,
  ) async {
    ...
    await tester.pumpWidget(
      testApp(
        overrides: [
          ...
          workLogsForSelectedDateProvider.overrideWith(
            (ref) async => <WorkLog>[],
          ),
        ],
    ...
    await tester.pumpWidget(
      testApp(
        overrides: [
          selectedDateProvider.overrideWith((ref) => date),
          workLogsForSelectedDateProvider.overrideWith(
            (ref) async => [_seedWorkLog('Push Up', date)],
          ),
        ],
        child: const Scaffold(body: WorkLogPage()),
      ),
  ...
  testWidgets('Shows an error widget when the provider throws', (tester) async {
    final date = DateTime(2026, 5, 16);
    await tester.pumpWidget(
      testApp(
        overrides: [
